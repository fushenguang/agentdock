...
- [x] 0. `git checkout -b feat/add-payments-to-web-nextjs`

## 1. 依赖安装

在 `templates/web-nextjs/apps/web/` 目录下：

- [x] 1. 安装支付宝 SDK：`pnpm add alipay-sdk`（微信支付使用 node:crypto 自实现，无需第三方包）
- [x] 1. 安装二维码渲染：`pnpm add qrcode.react`（仅用于微信 QR code UI 渲染）
- [x] 1. 确认 `@tanstack/react-query` 已存在（无需重装）

## 2. 数据层（Supabase）

- [x] 2. 创建迁移文件 `supabase/migrations/YYYYMMDD_add_subscription_tables.sql`，包含：
...
- [x] 2. 在开发 Supabase 执行迁移：`supabase db push` 或在 Dashboard SQL Editor 执行
- [x] 2. 验证：在 Supabase Dashboard 查看三张表结构和 RLS 策略正确

## 3. TypeScript 类型 + 服务层

- [x] 3. 创建 `src/features/subscription/types.ts`（从 thefoolai `subscription.types.ts` 裁剪，移除业务特定字段，保留核心类型）
- [x] 3. 创建 `src/features/subscription/service.ts`（4 个函数：`getActiveSubscriptionPlans`, `getSubscriptionPlanByCode`, `getUserActiveSubscription`, `getUserSubscriptionHistory`）
- [x] 3. 创建 `src/features/subscription/payment-service.ts`（3 个函数：`createPaymentRecord`, `getPaymentByOrderNo`, `getUserPayments`）
- [x] 3. 创建 `src/features/subscription/payment-helpers.ts`（`generateOrderNumber` 格式 `WN{timestamp}{4位随机}`）
- [x] 3. 创建 `src/lib/supabase/admin.ts`（`createAdminClient()` 使用 `SUPABASE_SERVICE_ROLE_KEY`，仅服务端使用，加注释警告）

## 4. 支付宝集成（Route Handlers）

- [x] 4. 创建 `src/features/subscription/alipay/server.ts`：
...
- [x] 5. 创建 `src/lib/wechat-pay/types.ts`：`WechatPayConfig` 接口定义（见 spec D3）
- [x] 5. 创建 `src/lib/wechat-pay/crypto.ts`（使用 `node:crypto`，实现以下函数）：
...
- [x] 5. 创建 `src/lib/wechat-pay/client.ts`：
  - `wxpayRequest<T>(params)` — 用内置 `fetch` 发起请求，自动附加 Authorization header
- [x] 5. 创建 `src/features/subscription/wechat/server.ts`：
...
- [x] 5. 创建 `src/features/subscription/wechat/client.ts`：
  - `initiateWechatPayment(paymentId, userId)` → 返回 `{ type: 'qrcode' | 'redirect', codeUrl?, h5Url? }`
- [x] 5. 创建 `src/app/api/payments/wechat/create/route.ts`：
  - PC → Native；开发环境移动端也走 Native（D3）
- [x] 5. 创建 `src/app/api/payments/wechat/query/route.ts`
- [x] 5. 创建 `src/app/api/payments/wechat/notify/route.ts`：
  - `const rawBody = await req.text()`（保持原始字节用于验签）
  - 验签 → 解密 resource → 幂等检查 → 更新 DB → 返回 `{ code: 'SUCCESS' }`

## 6. TanStack Query Hooks

- [x] 6. 创建 `src/features/subscription/hooks.ts`，提供：
...
- [x] 7. 创建 `src/app/[locale]/(protected)/pricing/page.tsx`：
  - 月付/年付 Tabs，从 `useSubscriptionPlans()` 读取，当前订阅高亮，点击跳转 checkout
- [x] 7. 创建 `src/app/[locale]/(protected)/payment/checkout/page.tsx`：
  - URL params 验证 → 计划摘要 → 支付方式选择（支付宝/微信 RadioGroup）→ 优惠码 Input → 确认支付 Button
- [x] 7. 创建 `src/app/[locale]/(protected)/payment/[paymentId]/page.tsx`：
  - paid → 成功卡片；failed → 失败卡片；pending → 微信时显示二维码（`<QRCodeSVG code_url />`）+ 3s 轮询
- [x] 7. 创建 `src/app/[locale]/(protected)/settings/subscription/page.tsx`：
  - 订阅状态卡片 + 支付历史列表 + 取消续费按钮（AlertDialog 确认）
- [x] 7. 在 settings sidebar 导航中添加"订阅与账单"入口

## 8. 公共组件（移植 + 适配）

- [x] 8. 创建 `src/features/subscription/components/upgrade-button.tsx`（`useNavigate` → `useRouter().push`）
- [x] 8. 创建 `src/features/subscription/components/pro-badge.tsx`（原样移植）
- [x] 8. 创建 `src/features/subscription/components/pro-feature-comparison.tsx`（`useTranslation(react-i18next)` → `useTranslations(next-intl)`）
- [x] 8. 创建 `src/features/subscription/components/quota-warning-banner.tsx`（移植 + 适配 next/navigation）
- [x] 8. 创建 `src/features/subscription/components/index.ts`（统一导出）

## 9. i18n

- [x] 9. 在 `src/i18n/messages/zh.ts` 添加 `pricing.*` / `subscription.*` / `payment.*` keys（中文）
- [x] 9. 在 `src/i18n/messages/en.ts` 添加对应英文 keys

## 10. 配置文件更新

- [x] 10. 更新 `templates/web-nextjs/apps/web/.env.local.example`，新增支付宝和微信支付的所有 env（含注释说明获取方式和沙箱配置）
