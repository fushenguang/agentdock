...
export const scaffoldProjectTool: McpTool = {
  ...
  description:
    "Scaffold a new AgentDock project from a template into the specified directory",
  inputSchema: {
    type: "object",
    properties: {
      name: {
      ...
      targetDir: {
        type: "string",
        description:
          "Absolute path to the target directory. Defaults to cwd/<name>.",
      },
      packageManager: {
        type: "string",
  ...
  async handler(input) {
    const { name, template: templateId, targetDir, packageManager } = input as {
      name: string;
      template: string;
      targetDir?: string;
...
export const getTemplateSchemaTool: McpTool = {
  ...
  description:
    "Get the full metadata and resolved dependency schema for a specific template",
  inputSchema: {
    type: "object",
    properties: {
      id: {
